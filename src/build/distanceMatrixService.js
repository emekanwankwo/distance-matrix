 ...
 */
const getDistanceMatrixWithTraffic = async (apiKey, origins, destinations, departure_time='now') => {
  // Check if there are multiple origins and/or destinations
  let mode = 'driving';
  let tripOrigins = origins, tripDestinations = destinations;
  const reducer = (accumulator, currentValue) => `${accumulator}|${currentValue}`;
  ...
  const distanceUrl = `https://maps.googleapis.com/maps/api/distancematrix/json?units=imperial&origins=${tripOrigins}&destinations=${tripDestinations}&mode=${mode}&departure_time=${departure_time}&key=${apiKey}`
    // Use google distance matrix API
    //@TODO: Tailor response to be more specific
